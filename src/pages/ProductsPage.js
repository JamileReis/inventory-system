  ...
  const handleAddComp = async (e) => {
    ...
    await dispatch(
        createComposition({
          ...
          rawMaterialId: Number(compForm.rawMaterialId),
          quantityRequired: Number(compForm.quantityRequired)
        })
    );
  ...
  return (
      <>
        ...
        <Modal show={showComp} onHide={() => setShowComp(false)} size="lg">
          <Modal.Header closeButton>
            <Modal.Title>
              Compositions — {selectedProd?.name}
            </Modal.Title>
          </Modal.Header>
            ...
            <Form onSubmit={handleAddComp} className="mb-3">
              <div className="d-flex gap-2">
                ...
                <Form.Control
                    ...
                    value={compForm.quantityRequired}
                    onChange={e =>
                        setCompForm({
                          ...compForm,
